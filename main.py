...
# BoT SeT 
TELEGRAM_BOT_TOKEN  ="8328014435:AAF6oFtIBfa3d1HYdINjUmqCs150LIHsP1I"
...
bot = telebot.TeleBot("8328014435:AAF6oFtIBfa3d1HYdINjUmqCs150LIHsP1I")
...
def execute_invite_command(client, target_id, user_id, thread_index):
    try:
        log_action("INVITE_START", user_id, f"Client {client['name']} ({thread_index}) inviting {target_id}")
        
        # Aqui você coloca seu código que convida para squad
        # Exemplo fictício:
        client.send_invite(target_id) 
        
        log_action("INVITE_SUCCESS", user_id, f"Client {client['name']} ({thread_index}) invited {target_id}")
        return True
    except Exception as e:
        log_action("INVITE_FAIL", user_id, f"Client {client['name']} ({thread_index}) Error: {str(e)}")
        return False
...
if not ACCOUNTS:
    ACCOUNTS = [{'id': '4293340713', 'password': '8C9E7DDADAE1BCE5CEFA917DD8F3E2E3E8A4488261DACBE4A8ABDB592ACBA99F'}]
...
# LaG MsG SeT
@bot.message_handler(func=lambda message: is_command(message, 'crash'))
...
@bot.message_handler(func=lambda message: is_command(message, 'invite'))
    ...
    try:
        ...
        if len(parts) < 2:
            bot.send_message(message.chat.id, 
                ...
                "📝 <b>Example:</b> <code>/invite 123456789</code>", 
                reply_to_message_id=message.message_id)
            return
        ...
        temp_message_id = send_temp_message(message.chat.id, 
            f"⏳ <b>Inviting player to squad...</b>\n"
            f"👤 Target: <code>{target_id}</code>",
            reply_to_message_id=message.message_id)
        
        clients_list = get_random_accounts(4)  # usa 4 contas, pode mudar
        ...
        for i, client in enumerate(clients_list, 1):
            thread = threading.Thread(target=lambda c=client, idx=i: execute_invite_command(c, target_id, user_id, idx))
            threads.append(thread)
            thread.start()
        ...
        bot.send_message(message.chat.id, 
            f"✅ <b>Squad invitations completed</b>\n"
            f"👤 Target: <code>{target_id}</code>\n"
            f"🔧 Accounts used: {len(clients_list)}",
            reply_to_message_id=message.message_id)
        ...
        if temp_message_id:
            delete_message(message.chat.id, temp_message_id)
        bot.send_message(message.chat.id, f"❌ <b>An error occurred:</b> {str(e)}", reply_to_message_id=message.message_id)
        log_action("INVITE_COMMAND_ERROR", user_id, str(e))
...
# HeX MsG SeT
@bot.message_handler(func=lambda message: is_command(message, 'x'))
...
def handle_spamvip_command(message):
    ...
    try:
        ...
        if not ChEck_Commande(uid):
            bot.send_message(
                message.chat.id,
                "❌ <b>player_id Invalid</b>",
                reply_to_message_id=message.message_id
            )
            return

        # Controle de tentativas
        attempts_remaining = get_user_attempts(user_id, 'spamvip')
        if attempts_remaining <= 0 and not has_unlimited_attempts(user_id):
            bot.send_message(
                message.chat.id,
                (
        ...
        # Consome tentativa (se não for VIP)
        if not use_attempt(user_id, 'spamvip') and not has_unlimited_attempts(user_id):
            bot.send_message(
                message.chat.id,
                (
                    "❌ <b>No attempts are available!</b>\n"
        ...
        # Envia mensagem inicial de status
        bot.send_message(
            message.chat.id,
            f"🚀 <b>Starting SPAMVIP on:</b> <code>{uid}</code>\n"
            f"⏳ Sending 3 requests with 5s delay...",
            reply_to_message_id=message.message_id
        )

        success_count = 0
        for i in range(3):
            url = f"https://ff-ave4.onrender.com/spm?uid={uid}"
            ...
            try:
                ...
                if data.get("status") == "success":
                    ...
                    bot.send_message(
                        message.chat.id,
                        f"✅ <b>Request {i+1}/3 sent successfully!</b>\n"
                ...
                bot.send_message(
                    message.chat.id,
                    f"❌ <b>Error on request {i+1}/3:</b> {str(e)}",
                    reply_to_message_id=message.message_id
                )

            # Aguarda 5 segundos antes da próxima
            if i < 2:
                time.sleep(5)

        if has_unlimited_attempts(user_id):
            remaining_text = "♾️ <b>Unlimited attempts</b>"
        else:
            remaining_text = f"🔄 <b>Remaining attempts:</b> {get_user_attempts(user_id, 'spamvip')}/3"

        bot.send_message(
            message.chat.id,
            f"🎉 <b>SPAMVIP Completed!</b>\n"
            f"📡 Successful requests: <b>{success_count}/3</b>\n"
            f"{remaining_text}",
            reply_to_message_id=message.message_id
        )
        ...
        log_action("SPAMVIP_ERROR", user_id, str(e))        
#StOp MsG SeT
